...
public interface LancamentoRepository extends JpaRepository<Lancamento, Long> {
    ...
    List<Lancamento> findByCompetenciaAndFuncionarioId(@Param("funcionarioId") Long funcionarioId, @Param("dataInicial") Timestamp dataInicial, @Param("dataFinal") Timestamp dataFinal);

    @Query(nativeQuery = true, value = "SELECT lanc.* FROM lancamento lanc WHERE lanc.centro_custo_id = :centroCustoId and lanc.data between :dataInicial and :dataFinal and lanc.tipo <> 'REGISTRO_INICIAL' and lanc.ativo = true order by lanc.funcionario_id ASC, lanc.data ASC")
    List<Lancamento> findByCompetenciaAndCentroCusto(@Param("centroCustoId") Long centroCustoId, @Param("dataInicial") Timestamp dataInicial, @Param("dataFinal") Timestamp dataFinal);
